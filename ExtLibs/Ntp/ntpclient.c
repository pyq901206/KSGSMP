...
static NtpGetTimeEvent ntpGetTimeEventFun;

static void set_time(struct ntptime *new)
{
	ntpGetTimeEventFun(new->coarse - JAN_1970);		
#if 0	
...
#define sizeof_incoming (sizeof incoming_word)
	...
	for (;;) {
		...
		if (pack_len<0) {
			...
			printf("udp_arrival_ntp.coarse = %u\r\n",udp_arrival_ntp.coarse);
...
#else
int NtpStart(char *ntpserver,int interal,NtpGetTimeEvent cbfun)
{
	int usd;  /* socket */
	int c;
	/* These parameters are settable from the command line
	   the initializations here provide default behavior */
	short int udp_local_port=0;   /* default of 0 means kernel chooses */
	char hostname[128] = {0};          /* must be set */
	int initial_freq;             /* initial freq value to use */
	struct ntp_control ntpc;
	#ifdef ENABLE_DEBUG
	//debug = 1;
	...
	ntpc.set_clock=1;
	ntpc.probe_count=0;           /* default of 0 means loop forever */
	ntpc.cycle_time= interal;          /* seconds */
	ntpc.goodness=0;
	ntpc.cross_check=1;
	sprintf(hostname,"%s",ntpserver);
